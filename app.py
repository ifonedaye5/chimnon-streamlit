            ...
            # ===== Có sheet knockout: dùng cấu hình ko_id / slot_home_from / slot_away_from =====
                ...
                try:
                    ...
                    for g in groups:
                        ...
                        # Lưu vào dict: A1 -> Tên đội, A2 -> Tên đội...
                ...
                # 2. Tính toán Winner/Loser của từng trận (Làm phẳng danh sách thắng thua)
                # Quan trọng: Sort theo match_id để đảm bảo trận trước (M201) được tính trước trận sau (M301)
                mm = mdf.copy()
                if "match_id" in mm.columns:
                    mm = mm.sort_values("match_id")
                ...
                # Hàm helper: resolve tên đội kể cả khi nó là "Winner Mxxx"
                def _resolve_recursive(tid: str) -> str:
                    tid = str(tid).strip()
                    # a) Nếu là Slot cơ sở (A1, B2)
                    if tid in slot_to_team:
                        return slot_to_team[tid]
                    
                    # b) Nếu là tham chiếu "Winner M..." (ví dụ home_team_id của M301 là Winner M201)
                    if tid.upper().startswith("WINNER "):
                        mid_ref = _norm_mid(tid.split()[-1])
                        if mid_ref in win_by_match:
                            return win_by_match[mid_ref]
                    
                    # c) Nếu là tham chiếu "Loser M..."
                    if tid.upper().startswith("LOSER "):
                        mid_ref = _norm_mid(tid.split()[-1])
                        if mid_ref in lose_by_match:
                            return lose_by_match[mid_ref]
                            
                    # d) Cuối cùng tra bảng tên đội gốc
                ...
                for _, r in mm.iterrows():
                    ...
                    # Lấy ID đội chủ nhà/khách từ dữ liệu match (có thể là A1, hoặc Winner M201...)
                    home_raw = str(r.get("home_team_id", "")).strip()
                    away_raw = str(r.get("away_team_id", "")).strip()
                    
                    # Resolve ra tên thật (QUAN TRỌNG: để M301 biết tên đội thắng M201)
                    hname = _resolve_recursive(home_raw)
                    aname = _resolve_recursive(away_raw)
                    ...
                    if pd.notna(hg) and pd.notna(ag):
                        if hg > ag:
                            ...
                            if pen in ("home", "h"):
                                ...
                                winner, loser = aname, hname
                            # Nếu hòa mà ko có pen -> code sẽ không xác định được winner -> giữ nguyên placeholder
                    
                    if winner and loser:
                        win_by_match[mid] = winner
                        lose_by_match[mid] = loser

                # 3. Hàm hiển thị cuối cùng cho sơ đồ nhánh
                def resolve_slot_final(s: str) -> str:
                    s = str(s).strip()
                    if not s: return ""
                    S = s.upper()
                    ...
                    # Winner Mxxx / Loser Mxxx
                    if S.startswith("WINNER "):
                        mid = _norm_mid(s.split()[-1])
                        return win_by_match.get(mid, s) # Trả về tên đội nếu có, không thì trả về "Winner M..."
                ...
                # Lọc lấy các vòng có trong dữ liệu
                ...
                for i, rn in enumerate(rounds_present):
                    with cols[i]:
                        ...
                        for _, rr in subr.iterrows():
                            # Dùng hàm mới để resolve tên
                            home = resolve_slot_final(rr.get("slot_home_from", ""))
                            away = resolve_slot_final(rr.get("slot_away_from", ""))
                            
                            mid = str(rr.get("match_id", "")).strip()
                            score_html = "vs"
                            meta_line = ""
                            status_html = ""
                            note_html = rr.get("notes","") or ""

                            # Lấy thông tin chi tiết trận đấu (nếu có trong matches)
                            ...
                            if not got.empty:
                                ...
                                parts = [x for x in [date, time_] if x] # bỏ venue cho gọn nếu muốn
                            ...
                            <div style='border:1px solid #e9ecef;border-radius:10px;padding:8px 10px;margin-bottom:8px;background:#fff;'>
                              <div style='display:flex;justify-content:space-between;gap:8px;font-size:14px;'>
                                <div style='flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600;'>{home}</div>
                                <div style='font-weight:800;background:#f8f9fa;padding:0 6px;border-radius:4px;'>{score_html}</div>
                                <div style='flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:right;font-weight:600;'>{away}</div>
                              </div>
