            ...
            if "player_id" in ev.columns and "player_id" in players_df.columns:
                ...
                if not cards.empty:
                    ...
                    st.info("Chưa có sự kiện thẻ nào.")
                            ...
                            # =========================
                    # 🤝 FAIR PLAY (TOÀN GIẢI) — kiểu A
                    # =========================
                    st.markdown("### 🤝 Đội Fair Play toàn giải")
            
                    ev2 = events_df.copy()
                    ev2.columns = [c.strip().lower() for c in ev2.columns]
            
                    # 1) Điểm fairplay theo đội (cả giải)
                    fp_all = compute_fairplay(ev2)  # điểm càng thấp càng tốt
            
                    # 2) Đếm thẻ để làm tie-break phụ (ít đỏ hơn tốt hơn, ít vàng hơn tốt hơn)
                    ...
                    # 3) Tính “đi xa hơn” (stage_depth) chỉ dùng khi BẰNG điểm FairPlay
                    #    - Ưu tiên lấy từ sheet knockout + match results
                    #    - stage_depth càng lớn = đi càng xa
                    round_order = ["1/8", "Tứ kết", "Bán kết", "Chung kết", "Tranh hạng 3"]
                    round_score = {r: i for i, r in enumerate(round_order, start=1)}  # 1..5
            
                    # map team_name <-> team_id để quy đổi
                    tdf2 = teams_df.copy()
                    tdf2.columns = [c.strip().lower() for c in tdf2.columns]
                    teamid_to_name = dict(zip(
                        tdf2.get("team_id", pd.Series(dtype=str)).astype(str).str.strip(),
                        tdf2.get("team_name", pd.Series(dtype=str)).astype(str).str.strip()
                    ))
                    name_to_teamid = {}
                    for tid, nm in teamid_to_name.items():
                        if nm and nm not in name_to_teamid:
                            name_to_teamid[nm] = tid
            
                    # ---- build slot_to_team A1/B2.. theo BXH hiện tại (giống tab2) ----
                    slot_to_team = {}
                    try:
                        ...
                        groups = (
                            t_all.get("group", pd.Series(dtype=str)).astype(str).str.upper().unique().tolist()
                        )
                        groups = [g for g in groups if g and g.lower() != "nan"]
                        ...
                        for g in groups:
                            ...
                            if sort_cols:
                                table = table.sort_values(by=sort_cols, ascending=asc_flags).reset_index(drop=True)
                            if "Hạng" in table.columns:
                                table = table.drop(columns=["Hạng"])
                            table.insert(0, "rank", range(1, len(table) + 1))
                            table = table.rename(columns={"Đội": "team_name"})
                            for _, rr in table.iterrows():
                                key = f"{g}{int(rr['rank'])}"
                                slot_to_team[key] = str(rr.get("team_name") or "")
                    except Exception:
                        pass
            
                    def _norm_round(val: str) -> str:
                        if not isinstance(val, str):
                            return ""
                        v = val.strip().lower()
                        maps = {
                    ...
                    # ---- Map winner/loser theo match_id (bản “đã fix Winner Mxxx”) ----
                    mdf2 = matches_df.copy()
                    mdf2.columns = [c.strip().lower() for c in mdf2.columns]
                    win_by_match, lose_by_match = {}, {}
                    ...
                    def resolve_ref(tid: str) -> str:
                        ...
                        if S.startswith("LOSER "):
                            ...
                            return lose_by_match.get(mid, tid)
                        # nếu là team_id thật
                        return teamid_to_name.get(tid, tid)
                    ...
                    def decide_winner_loser(row):
                        ...
                        if hg > ag: return (hname, aname)
                        if hg < ag: return (aname, hname)
            
                        if pen in ("home","h"): return (hname, aname)
                        if pen in ("away","a"): return (aname, hname)
                        return None
                    ...
                    def resolve_slot_name(s: str) -> str:
                        s = str(s or "").strip()
                        if not s:
                            return ""
                        S = s.upper()
                        ...
                        if S.startswith("M") and S[1:].isdigit():
                            return win_by_match.get(S, s)
                        return teamid_to_name.get(s, s)
            
                    stage_depth = {}  # team_id -> depth
            
                    if knockout_df is not None and not knockout_df.empty:
                        ko2 = knockout_df.copy()
                        ko2.columns = [c.strip().lower() for c in ko2.columns]
                        ko2["round_norm"] = ko2.get("round", "").apply(_norm_round)
            
                        for _, rr in ko2.iterrows():
                            rn = rr.get("round_norm","")
                            base = round_score.get(rn, 0)
                            mid = str(rr.get("match_id","")).strip()
                            if base <= 0:
                                continue
            
                            home_nm = resolve_slot_name(rr.get("slot_home_from",""))
                            away_nm = resolve_slot_name(rr.get("slot_away_from",""))
            
                            home_tid = name_to_teamid.get(home_nm, "")
                            away_tid = name_to_teamid.get(away_nm, "")
            
                            # chỉ “đi xa hơn” khi có team_id match được
                            for tid in [home_tid, away_tid]:
                                if tid:
                                    stage_depth[tid] = max(stage_depth.get(tid, 0), base)
            
                            # bonus nhẹ cho đội thắng ở round đó (đi sâu hơn 0.5)
                            if mid and mid in win_by_match:
                                wname = win_by_match.get(mid, "")
                                wtid = name_to_teamid.get(wname, "")
                                if wtid:
                                    stage_depth[wtid] = max(stage_depth.get(wtid, 0), base + 0.5)
            
                    # 4) Lập bảng fairplay theo đội
                    team_ids = sorted(set(teamid_to_name.keys()) | set(fp_all.keys()))
                    rows = []
                    for tid in team_ids:
                        rows.append({
                            "team_id": tid,
                            "Đội": teamid_to_name.get(tid, tid),
                            "Thẻ vàng": c_y.get(tid, 0),
                            "Đỏ gián tiếp (2V)": c_sy.get(tid, 0),
                            "Đỏ trực tiếp": c_r.get(tid, 0),
                            "Vàng+Đỏ": c_ypr.get(tid, 0),
                            "Điểm FairPlay": int(fp_all.get(tid, 0)),
                            # chỉ dùng khi bằng điểm fairplay
                            "Đi xa hơn": float(stage_depth.get(tid, 0.0)),
                        })
                    ...
                    # Sort kiểu A:
                    # 1) FairPlay ↑
                    # 2) Đi xa hơn ↓ (chỉ ý nghĩa khi bằng FP)
                    # 3) Ít đỏ hơn ↑ (đỏ trực tiếp + 2V + vàng+đỏ)
                    # 4) Ít vàng hơn ↑
                    fp_df["Tổng đỏ"] = (
                        fp_df["Đỏ trực tiếp"].astype(int)
                        + fp_df["Đỏ gián tiếp (2V)"].astype(int)
                        + fp_df["Vàng+Đỏ"].astype(int)
                    )
            
                    fp_df = fp_df.sort_values(
                        by=["Điểm FairPlay", "Đi xa hơn", "Tổng đỏ", "Thẻ vàng", "Đội"],
                        ascending=[True, False, True, True, True]
                    ).reset_index(drop=True)
            
                    fp_df.insert(0, "Hạng", range(1, len(fp_df) + 1))
            
                    show_cols = ["Hạng","Đội","Điểm FairPlay","Đi xa hơn","Thẻ vàng","Đỏ gián tiếp (2V)","Đỏ trực tiếp","Vàng+Đỏ"]
                    st.dataframe(fp_df[show_cols], use_container_width=True, hide_index=True)
